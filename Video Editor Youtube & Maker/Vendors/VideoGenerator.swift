...
public class VideoGenerator: NSObject {
     ...
     */
    open func generate(videoName: String = "VideoTest", withImages _images: [UIImage], andAudios _audios: [URL], andType _type: VideoGeneratorType, duration: Float, _ progress: @escaping ((Progress) -> Void), outcome: @escaping (Result<URL, Error>) -> Void) {
                ...
                if let videoWriter = VideoGenerator.current.videoWriter {
                    ...
                    if videoWriter.startWriting() {
                        ...
                        videoWriterInput.requestMediaDataWhenReady(on: media_queue, using: { () -> Void in
                            ...
                            var frameDuration = CMTime(seconds: Double(duration), preferredTimescale: 1)
                            if _audios.count > 0 {
                                frameDuration = CMTime(seconds: VideoGenerator.current.duration, preferredTimescale: 1)
                            }
                            print("======================frame duration: \(frameDuration.seconds)")
                            ...
                            while (videoWriterInput.isReadyForMoreMediaData && frameCount < numImages) {
                                ...
                                if VideoGenerator.current.type == .single {
                                    ...
                                    if VideoGenerator.current.type == .multiple {
                                        ...
                                        elapsedTime += frameDuration.seconds / Double(total_Images)
                                    }
                                }
                            ...
                            videoWriter.finishWriting { () -> Void in
                                if self?.audioURLs.isEmpty == true {
                                    ...
                                    let outputURL = documentDirectory.appendingPathComponent("\(videoName).mp4")
                                    self?.deleteFile(pathURL: outputURL, completion: {
                                        try FileManager.default.moveItem(at: videoOutputURL, to: outputURL)
                                    })
                                    print("finished")
                                    DispatchQueue.main.async {
                                    ...
                                    //                    let newPath = DataManager.shared.getVideoUrl()
